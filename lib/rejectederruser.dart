...
class rejected_user extends StatefulWidget {
  final String title;


  const rejected_user(String s,  {required Key key,required this.title}) : super(key: key);
  @override
  _rejected_userState createState() => _rejected_userState();
}

final FirebaseMessaging _fcm = FirebaseMessaging.instance;

class _rejected_userState extends State<rejected_user> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Rejected Errands',
        style: TextStyle(
          color: Colors.brown
        ),),
        centerTitle: true,
  ...
  Widget build(BuildContext context) {
      ...
      FutureBuilder(
        //  future: FirebaseAuth.instance.currentUser(),
          builder: (context,AsyncSnapshot<FirebaseAuth> snapshot){
            final Stream<QuerySnapshot> _userStream =  FirebaseFirestore.instance.collection('Add_Errands').where('userId', isEqualTo: userID)
                .where('Status', isEqualTo: 'Rejected').orderBy('Date',descending:true).snapshots();
    ...
    return StreamBuilder(
                    ...
                    builder: (BuildContext context,AsyncSnapshot snapshot){
                      ...
                      if (snapshot.hasData == false) {
                        return Center(
                          child: Text('No errand has been rejected'),
                        );
                      }
                      else {
                        ...
                        dynamic phone;
                              ...
                              phone =  Text(snapshot.data!.docs[index]['Phone_number']);
                              return ListTile(
